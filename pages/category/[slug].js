...
const maxResult= 1;
//slug.js is a category name i.e. sneakers
...
useEffect(()=>{
  ...
  const {data, error, isLoading} = useSWR(`/api/products?populate=*&[filters][categories][slug][$eq]=${slug}&pagination[page]=${pageIndex}&pagination[pageSize]=${maxResult}`, fetchDataFromApi,{
    fallbackData: products,//the previous data will remain while loading and will then be removed when the next page data comes
  })
  return (
    <div className="w-full md:py-20 relative">
        <Wrapper>
   
    <div className='text-center min-w-[800px] mx-auto mt-8 md:mt-0'>
...
export async function getStaticPaths(){
  ...
  const paths= category.data.map((c)=>({//here ()parenthesis are used to return and {}are used to return inside an object
    params:{
